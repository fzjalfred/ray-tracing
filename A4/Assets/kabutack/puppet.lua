...
rootNode:rotate('X', 23)
rootNode:translate(6, -2, -15)
...
-- rootNode:add_child(poly)
-- poly:set_material(lightBlue)


---------------------------------------torso--------------------------------------
torso = gr.sphere('torso')
torso:set_material(red)
torso:translate(-5.0, 3.0, 4.0)
rootNode:add_child(torso)

torso_decorate = gr.mesh('torso_decorate', 'torso_decorate.obj')
torso_decorate:set_material(golden)
torso:add_child(torso_decorate)

-- torso_black_body = gr.mesh('torso_black_body', 'torso_black_body.obj')
...
neckJoint:translate(0.0, 0.0, 0.0)
...
face:translate(-1.5, 0.0, 0.0)
neckNode:add_child(face)


head = gr.sphere('head')
head:set_material(red)
head:translate(-3.0, 3.0, 0.0)
head:scale(2.0,2.0,2.0)
face:add_child(head)

-- head_decorate = gr.mesh('head_decorate', 'head_decorate.obj')
-- head_decorate:set_material(golden)
-- head:add_child(head_decorate)

-- head_black_body = gr.mesh('head_black_body', 'head_black_body.obj')
-- head_black_body:set_material(black)
-- head:add_child(head_black_body)

-- eyes = gr.mesh('eyes', 'eyes.obj')
-- eyes:set_material(black)
-- head:add_child(eyes)

nose = gr.sphere('nose')
nose:set_material(lightBlue)
nose:translate(-3.0, 0.0, 0.0)
head:add_child(nose)
...
-- rightUpperArm = gr.mesh('ArmUp', 'rightUpperArm.obj')
...
-- rightLowerArm = gr.mesh('ArmLower', 'rightLowerArm.obj')
...
-- rightHand = gr.mesh('hand', 'rightHand.obj')
...
-- leftUpperArm = gr.mesh('ArmUp', 'leftUpperArm.obj')
...
-- leftLowerArm = gr.mesh('ArmLower', 'leftLowerArm.obj')
...
-- leftHand = gr.mesh('hand', 'leftHand.obj')
...
-- rightLegUp = gr.mesh('LegUp', 'rightLegUp.obj')
...
-- rightLegLower = gr.mesh('LegLower', 'rightLegLower.obj')
...
-- rightFoot = gr.mesh('foot', 'rightFoot.obj')
...
-- leftLegUp = gr.mesh('LegUp', 'leftLegUp.obj')
...
-- leftLegLower = gr.mesh('LegLower', 'leftLegLower.obj')
...
-- leftFoot = gr.mesh('foot', 'leftFoot.obj')
...
gr.render(rootNode, 'kabutack.png', 256, 256,
